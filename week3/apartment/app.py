...
print('Features are: ', ['rooms', 'area', 'pop', 'pop_dens', 'frg_pct', 'emp', 'tax_income', 'employment_ratio', 'distance_to_center'])
random_forest_model
...
def predict_apartment(rooms, area, distance_to_center, town):
    df_bfs_data['employment_ratio'] = df_bfs_data['emp'] / df_bfs_data['pop']
    bfs_number = locations[town]
    df = df_bfs_data[df_bfs_data['bfs_number']==bfs_number].copy()
    df.reset_index(inplace=True)
    df.loc[0, 'rooms'] = rooms
    df.loc[0, 'area'] = area
    df.loc[0, 'distance_to_center'] = distance_to_center
    if len(df) != 1: # if there are more than two records with the same bfs_number return -1
        return -1
    prediction = random_forest_model.predict(df[['rooms', 'area', 'pop', 'pop_dens', 'frg_pct', 'emp', 'tax_income', 'employment_ratio', 'distance_to_center']])
    return np.round(prediction[0], 0)

predict_apartment(3, 100, 1.2, 'Zürich')

# Create the Gradio interface
iface = gr.Interface(
    fn=predict_apartment,
    inputs=["number", "number", "number", gr.Dropdown(choices=locations.keys(), label="Town", type="value")],
    outputs=[gr.Number()],
    examples=[[4.5, 120, 1.2, "Dietlikon"], [3.5, 60, 0.1, "Winterthur"]]
)
